...
import { Brain, ChevronDown, ChevronUp, CalendarDays, Wand2 } from 'lucide-react';
...
export function AiFeedback() {
  ...
  useEffect(() => {
    API.get('/ai/feedback').then(r => setFeedbacks(r.data)).finally(() => setBusy(false));
  }, []);

  if (busy) return <div className="flex justify-center py-24"><Spinner size="lg" text="Loading feedback..."/></div>;
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {feedbacks.length === 0
        ? <EmptyState icon={Brain} title="No AI feedback yet"
        ...
        : <div className="space-y-3">
            {feedbacks.map(fb => {
              ...
              const scoreColor = fb.score >= 75 ? 'text-success-400' : fb.score >= 50 ? 'text-warning-400' : 'text-danger-400';
              return (
                <div key={fb.id} className="card overflow-hidden">
                  <button onClick={() => setOpen(isOpen ? null : fb.id)}
...
export function StudyPlan() {
  ...
  const [open, setOpen]            = useState(null);
  const [form, setForm]            = useState({ companyName: 'TCS', planDays: 5, weakTopics: '' });

  const COMPANIES = ['TCS', 'Infosys', 'Wipro', 'Zoho', 'Amazon', 'Accenture', 'Cognizant', 'Capgemini'];

  useEffect(() => {
    if (loaded) return;
    API.get('/ai/study-plans').then(r => {
      setPlans(r.data);
  ...
  const generate = async () => {
    setGenerating(true);
    try {
      const weakTopics = form.weakTopics ? form.weakTopics.split(',').map(s => s.trim()).filter(Boolean) : [];
      const r = await API.post('/ai/study-plan', { ...form, planDays: Number(form.planDays), weakTopics });
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        ...
        <p className="section-sub">Generate a personalized day-by-day preparation plan</p>
      </div>
      ...
      <div className="card p-6 border-brand-500/20 bg-brand-500/5">
        ...
        <div className="grid sm:grid-cols-3 gap-4 mb-5">
          <div>
            ...
            <select className="input" value={form.companyName} onChange={e => setForm({...form, companyName: e.target.value})}>
              {COMPANIES.map(c => <option key={c}>{c}</option>)}
            </select>
          </div>
          <div>
            <label className="label">Plan Duration</label>
            <select className="input" value={form.planDays} onChange={e => setForm({...form, planDays: e.target.value})}>
              {[3, 5, 7, 10, 14].map(d => <option key={d} value={d}>{d} Days</option>)}
            </select>
          </div>
          <div>
            <label className="label">Weak Topics (comma-sep)</label>
            <input className="input" placeholder="SQL Joins, OOP, Arrays"
              value={form.weakTopics} onChange={e => setForm({...form, weakTopics: e.target.value})}/>
          </div>
        </div>
          ...
          {plans.map(p => (
            <div key={p.id} className="card overflow-hidden">
              <button onClick={() => setOpen(open === p.id ? null : p.id)}
                ...
                {open === p.id ? <ChevronUp size={18} className="text-gray-500"/> : <ChevronDown size={18} className="text-gray-500"/>}
              </button>
              {open === p.id && (
                <div className="px-4 pb-4 border-t border-gray-800 pt-4">
...
export function Profile() {
  ...
  useEffect(() => {
    API.get('/dashboard/summary').then(r => setData(r.data)).finally(() => setBusy(false));
  }, []);
